...
def recommend_portfolio(intent_request):
    ...
    if age is not None:
        if age < 0 or age > 65:
            return build_validation_result(
                False,
                "age",
                "This retirement investment service is for those between the ages of 0 and 65, "
    ...
    if risk_level is None:
        return build_validation_result(
            True,
            "riskLevel",
            "Based on your risk appetite, we recommemd 100% bonds (AGG), 0% equities (SPY)"
        )
    elif risk_level == "Low":
            return build_validation_result(
                True,
                "riskLevel",
                "Based on your risk appetite, we recommend 60% bonds (AGG), 40% equities (SPY)"
            )
    elif risk_level == "Medium":
            return build_validation_result(
                True,
                "riskLevel",
                "Based on your risk appetite, we recommend 40% bonds (AGG), 60% equities (SPY)"
            )
    elif risk_level == "High":
            return build_validation_result(
                True,
                "riskLevel",
                "Based on your risk appetite, we recommend 20% bonds (AGG), 80% equities (SPY)"
            )
